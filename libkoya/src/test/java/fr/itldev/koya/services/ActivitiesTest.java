...
public class ActivitiesTest extends TestCase {
	...
	public void init() throws RestClientException, AlfrescoServiceException {
		 ...
		 */
		 ...
		 manager = validateInvitationAndLogin(USER_MANAGER,
		 managerInviteWrapper);
	 ...
	 userService.listActivities(admin);
	 }
	 ...
	 List<Activity> result = readFeedTimeout(u);
	
	 System.err.println(result.size() + " notifications");
	
	 for (Activity n : result) {
	 System.out.println(" " + n.getId() + " - " + n.getPostUserId()
	 + " - " + n.getActivityType() + "--" + n.getMessage());
	 }
	
	 }
	
	 private List<Activity> readFeedTimeout(User user)
	 throws InterruptedException {
	 List<Activity> result = userService.listActivities(user);
	
	 long t = System.currentTimeMillis();
	 long end = t + 60000;
	
	 while (result.size() == 0 && System.currentTimeMillis() < end) {
	 result = userService.listActivities(user);
	 Thread.sleep(5000);
	 System.out.print("+");
	 ...
	 private User validateInvitationAndLogin(String loginPrefix, KoyaInvite
	 iw) {
