...
public class DossierServiceImpl extends AlfrescoRestService implements DossierService {
    ...
    @Override
    public Dossier creerNouveau(Utilisateur user, Dossier dossier) throws AlfrescoServiceException {
    ...
    @Override
    public Dossier editer(Utilisateur user, Dossier dossier) throws AlfrescoServiceException {
     ...
     */
    @Override
    public List<Dossier> lister(Utilisateur user, Espace espace, String... filter) throws AlfrescoServiceException {
        String filtre = "";
        if (filter.length == 1) {
            filtre = filter[0];
        }
        ItlAlfrescoServiceWrapper ret = user.getRestTemplate().postForObject(getAlfrescoServerUrl() + REST_POST_LISTCHILD, espace, ItlAlfrescoServiceWrapper.class, filtre);
