...
public class CompanyService {
	...
	public CompanyProperties getProperties(Company c)
		...
		if (companyPropertiesNodeRef != null) {
			...
			for (ChildAssociationRef car : nodeService
				...
				cp.setLogoNodeRef(getLogo(c).toString());

				if (currentNodeRefType.equals(KoyaModel.TYPE_CONTACTITEM)) {
					try {
						ContactItem companyCi = new ContactItem();

						companyCi.setValue((String) nodeService.getProperty(
								currentNodeRef,
								KoyaModel.PROP_CONTACTITEM_VALUE));
					...
					contact.setTitle((String) nodeService.getProperty(
							currentNodeRef, KoyaModel.PROP_CONTACT_TITLE));

					// ========= get associated user informations
					List<AssociationRef> assocRefs = nodeService
							.getTargetAssocs(currentNodeRef,
					...
					contact.setFirstName((String) nodeService.getProperty(
							userNodeRef, ContentModel.PROP_FIRSTNAME));
					contact.setLastName((String) nodeService.getProperty(
							userNodeRef, ContentModel.PROP_LASTNAME));

					ContactItem contactMail = new ContactItem();
					contactMail.setType(ContactItem.TYPE_MAIL);
					contactMail.setValue((String) nodeService.getProperty(
							userNodeRef, ContentModel.PROP_EMAIL));

					contact.getContactItems().add(contactMail);

					// ========= get additionnal contacts =======
					for (ChildAssociationRef carContact : nodeService
							.getChildAssocs(currentNodeRef)) {
						NodeRef currentContactNodeRef = carContact
								.getChildRef();
						if (nodeService.getType(currentContactNodeRef).equals(
								KoyaModel.TYPE_CONTACTITEM)) {
							try {

								ContactItem contactCi = new ContactItem();
								contactCi
										.setValue((String) nodeService
												.getProperty(
														currentContactNodeRef,
														KoyaModel.PROP_CONTACTITEM_VALUE));
								contactCi
										.setType(Integer.valueOf((String) nodeService
												.getProperty(
														currentContactNodeRef,
														KoyaModel.PROP_CONTACTITEM_TYPE)));
								contact.getContactItems().add(contactCi);
							} catch (NumberFormatException
									| InvalidNodeRefException ex) {
								// silent exception catching : if error occurs
								// no contact created
							}
						}
					}
	...
	public NodeRef getLogo(Company c) {
		...
		String companyLogoQuery = "+PATH:\"/app:company_home/st:sites/cm:"
				+ c.getName()
				+ "/cm:koya-config/cm:company.properties/*\"  +@cm\\:name:"
				+ LOGO_FILE_NAME_PREFIX + "*";

		ResultSet rs = null;
		try {
			rs = searchService.query(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
					SearchService.LANGUAGE_LUCENE, companyLogoQuery);
		...
		// if not found returns koya default config logo

		try {
			rs = searchService.query(StoreRef.STORE_REF_WORKSPACE_SPACESSTORE,
					SearchService.LANGUAGE_XPATH, XPATH_DEFAULTCOMPANY_LOGO);
	 ...
	 * @throws KoyaServiceException
	 */
	public void commitProperties(String companyName,
			CompanyProperties compProperties) throws KoyaServiceException {
		Company c = koyaNodeService.getSecuredItem(
				siteService.getSite(companyName).getNodeRef(), Company.class);
	...
	protected NodeRef getKoyaConfigNodeRef(final NodeRef rootRef,
		...
		if (create && koyaConfigRef == null) {
			...
			QName assocQName = QName.createQName(DESC).createQName(
